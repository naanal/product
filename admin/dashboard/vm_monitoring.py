...
dump_directory = "/home/raja/Dashboards/naanalstack/product/admin/dashboard/static/"
...
def get_fixed_ip(info, neutron):
    try:
        ...
        for net in info.get('addresses', ''):
            tmp_dict = {}
            for nic in info.get('addresses')[net]:
                net_id = [x['id'] for x in neutron.list_networks(name=net)[
                    'networks'] if len(x)]
                tmp_dict['net-id'] = net_id[0]
                for port in info.get('addresses')[net][0]:
                    tmp_dict['v%s-%s-ip' %
                             (nic['version'], nic['OS-EXT-IPS:type'])] = nic['addr']
...
def json_dump_write(filename="vm_monitoring.json", data=None):
    ...
    try:
        while True:
            ...
            for ins in ins_list:
                health = 0
                vm_obj = {}
                instance, info, ip_list, bdm, extra = info_collection(
                    nova, cinder, ins.id)
                ...
                if (instance.status == 'ACTIVE'):
                    health = 1
                if ( instance.status == 'ACTIVE' and ( rdp_status == False or ping_status == False)):
                    health = 3
