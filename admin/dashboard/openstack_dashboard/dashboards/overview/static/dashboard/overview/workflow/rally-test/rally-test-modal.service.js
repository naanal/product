...
(function () {
  ...
  angular
    ...
    .factory(
      'horizon.dashboard.overview.workflow.rally-test.modal.service',
      RallyTestModalService
    );

  RallyTestModalService.$inject = [
    '$modal',
    '$window',
    'horizon.dashboard.overview.workflow.rally-test.modal-spec'
  ];

  function RallyTestModalService($modal, $window, modalSpec) {
    var service = {
      open: open
    };
    ...
    function open(launchContext) {
      ...
      var RallyTestModal = $modal.open(localSpec);
      ...
      return RallyTestModal.result.then(
