...
from openstack_dashboard.dashboards.virtual.containers import forms
from openstack_dashboard.dashboards.virtual.containers import tables
from openstack_dashboard.dashboards.virtual.containers import utils
from openstack_dashboard.dashboards.virtual.containers import views
...
CONTAINER_INDEX_URL = reverse('horizon:virtual:containers:index')
...
def invalid_paths():
    if not INVALID_PATHS:
        for x in (CONTAINER_NAME_1_QUOTED, CONTAINER_NAME_2_QUOTED):
            y = reverse('horizon:virtual:containers:index',
...
class SwiftTests(test.TestCase):
    ...
    def test_index_no_container_selected(self):
        ...
        self.assertTemplateUsed(res, 'virtual/containers/index.html')
    ...
    def test_create_container_get(self):
        res = self.client.get(reverse('horizon:virtual:containers:create'))
        self.assertTemplateUsed(res, 'virtual/containers/create.html')
    ...
    def test_create_container_post(self):
        for container in self.containers.list():
            ...
            res = self.client.post(
                reverse('horizon:virtual:containers:create'), formData)
            args = (utils.wrap_delimiter(container.name),)
            url = reverse('horizon:virtual:containers:index', args=args)
    ...
    def test_index_container_selected(self):
        ...
        res = self.client.get(
            reverse('horizon:virtual:containers:index',
    ...
    def test_upload(self):
        ...
        upload_url = reverse('horizon:virtual:containers:object_upload',
                             args=[container.name])

        res = self.client.get(upload_url)
        self.assertTemplateUsed(res, 'virtual/containers/upload.html')
        ...
        args = (utils.wrap_delimiter(container.name),)
        index_url = reverse('horizon:virtual:containers:index', args=args)
    ...
    def test_create_pseudo_folder(self):
        ...
        create_pseudo_folder_url = reverse('horizon:virtual:containers:'
                                           'create_pseudo_folder',
                                           args=[container.name])

        res = self.client.get(create_pseudo_folder_url)
        self.assertTemplateUsed(res,
                                'virtual/containers/create_pseudo_folder.html')
        self._test_invalid_paths(res)
        ...
        index_url = reverse('horizon:virtual:containers:index',
                            args=[utils.wrap_delimiter(container.name)])
        ...
        for container in self.containers.list():
            for obj in self.objects.list():
                ...
                download_url = reverse(
                    'horizon:virtual:containers:object_download',
    ...
    def test_copy_index(self):
        ...
        res = self.client.get(reverse('horizon:virtual:containers:object_copy',
                                      args=[self.containers.first().name,
                                            self.objects.first().name]))
        self.assertTemplateUsed(res, 'virtual/containers/copy.html')
        self.assertNotContains(res, INVALID_CONTAINER_NAME_1)
        self.assertNotContains(res, INVALID_CONTAINER_NAME_2)
    ...
    def test_copy(self):
        ...
        copy_url = reverse('horizon:virtual:containers:object_copy',
    ...
    def test_update_with_file(self):
        ...
        update_url = reverse('horizon:virtual:containers:object_update',
                             args=[container.name, obj.name])

        res = self.client.get(update_url)
        self.assertTemplateUsed(res, 'virtual/containers/update.html')
        self.assertContains(res, 'enctype="multipart/form-data"')
        self._test_invalid_paths(res)
    ...
    def test_view_container(self):
        for container in self.containers.list():
            ...
            view_url = reverse('horizon:virtual:containers:container_detail',
                               args=[container.name])
            res = self.client.get(view_url)

            self.assertTemplateUsed(res,
                                    'virtual/containers/container_detail.html')
        ...
        for container in self.containers.list():
            for obj in self.objects.list():
                ...
                api.swift.swift_get_object(IsA(http.HttpRequest),
                    ...
                    .AndReturn(obj)
                self.mox.ReplayAll()
                view_url = reverse('horizon:virtual:containers:object_detail',
                                   args=[container.name, obj.name])
                res = self.client.get(view_url)

                self.assertTemplateUsed(
                    res, 'virtual/containers/object_detail.html')
