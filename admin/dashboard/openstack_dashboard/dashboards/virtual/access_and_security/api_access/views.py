...
from openstack_dashboard.dashboards.virtual.access_and_security.api_access \
...
def download_ec2_bundle(request):
    ...
    template = 'virtual/access_and_security/api_access/ec2rc.sh.template'
    try:
...
def download_rc_file_v2(request):
    template = 'virtual/access_and_security/api_access/openrc_v2.sh.template'
...
def download_rc_file(request):
    template = 'virtual/access_and_security/api_access/openrc.sh.template'
    context = _get_openrc_credentials(request)
...
class CredentialsView(forms.ModalFormMixin, views.HorizonTemplateView):
    template_name = 'virtual/access_and_security/api_access/credentials.html'
...
class RecreateCredentialsView(forms.ModalFormView):
    form_class = virtual_forms.RecreateCredentials
    form_id = "recreate_credentials"
    modal_header = _("Recreate EC2 Credentials")
    template_name = \
        'virtual/access_and_security/api_access/recreate_credentials.html'
    submit_label = _("Recreate EC2 Credentials")
    submit_url = reverse_lazy(
        "horizon:virtual:access_and_security:api_access:recreate_credentials")
