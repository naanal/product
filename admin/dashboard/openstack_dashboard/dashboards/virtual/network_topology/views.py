...
from openstack_dashboard.dashboards.virtual.network_topology.instances \
    import tables as instances_tables
from openstack_dashboard.dashboards.virtual.network_topology.networks \
    import tables as networks_tables
from openstack_dashboard.dashboards.virtual.network_topology.ports \
    import tables as ports_tables
from openstack_dashboard.dashboards.virtual.network_topology.routers \
    import tables as routers_tables
from openstack_dashboard.dashboards.virtual.network_topology.subnets \
    import tables as subnets_tables

from openstack_dashboard.dashboards.virtual.instances import\
    console as i_console
from openstack_dashboard.dashboards.virtual.instances.tables import \
    STATUS_DISPLAY_CHOICES as instance_choices
from openstack_dashboard.dashboards.virtual.instances import\
    views as i_views
from openstack_dashboard.dashboards.virtual.instances.workflows import\
    create_instance as i_workflows
from openstack_dashboard.dashboards.virtual.networks.subnets import\
    views as s_views
from openstack_dashboard.dashboards.virtual.networks.subnets import\
    workflows as s_workflows
from openstack_dashboard.dashboards.virtual.networks.tables import \
    DISPLAY_CHOICES as network_display_choices
from openstack_dashboard.dashboards.virtual.networks.tables import \
    STATUS_DISPLAY_CHOICES as network_choices
from openstack_dashboard.dashboards.virtual.networks import\
    views as n_views
from openstack_dashboard.dashboards.virtual.networks import\
    workflows as n_workflows
from openstack_dashboard.dashboards.virtual.routers.ports.tables import \
    DISPLAY_CHOICES as ports_choices
from openstack_dashboard.dashboards.virtual.routers.ports.tables import \
    STATUS_DISPLAY_CHOICES as ports_status_choices
from openstack_dashboard.dashboards.virtual.routers.ports import\
    views as p_views
from openstack_dashboard.dashboards.virtual.routers.tables import \
    ADMIN_STATE_DISPLAY_CHOICES as routers_admin_choices
from openstack_dashboard.dashboards.virtual.routers.tables import \
    STATUS_DISPLAY_CHOICES as routers_status_choices
from openstack_dashboard.dashboards.virtual.routers import\
    views as r_views
...
class NTAddInterfaceView(p_views.AddInterfaceView):
    success_url = "horizon:virtual:network_topology:index"
    failure_url = "horizon:virtual:network_topology:index"

    def get_success_url(self):
        return reverse("horizon:virtual:network_topology:index")

    def get_context_data(self, **kwargs):
        context = super(NTAddInterfaceView, self).get_context_data(**kwargs)
        context['form_url'] = 'horizon:virtual:network_topology:interface'
        return context


class NTCreateRouterView(r_views.CreateView):
    template_name = 'virtual/network_topology/create_router.html'
    success_url = reverse_lazy("horizon:virtual:network_topology:index")
...
class InstanceView(i_views.IndexView):
    ...
    template_name = 'virtual/network_topology/iframe.html'
...
class NetworkTopologyView(views.HorizonTemplateView):
    template_name = 'virtual/network_topology/index.html'
...
class JSONView(View):
    ...
    def _get_servers(self, request):
        ...
        self.add_resource_url('horizon:virtual:instances:detail', data)
        return data
    ...
    def _get_networks(self, request):
        ...
        for network in neutron_networks:
            ...
            self.add_resource_url('horizon:virtual:networks:subnets:detail',
                                  obj['subnets'])
            networks.append(obj)
                ...
                try:
                    ...
                    for subnet in publicnet.subnets:
                        ...
                        self.add_resource_url(
                            'horizon:virtual:networks:subnets:detail', snet)
                        subnets.append(snet)
                except Exception:
                    subnets = []
                networks.append({
        ...
        self.add_resource_url('horizon:virtual:networks:detail',
                              networks)
    ...
    def _get_routers(self, request):
        ...
        self.add_resource_url('horizon:virtual:routers:detail', routers)
        return routers
    ...
    def _get_ports(self, request):
        ...
        self.add_resource_url('horizon:virtual:networks:ports:detail',
                              ports)
        return ports
