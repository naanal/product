...
from openstack_dashboard.dashboards.virtual.network_topology.views import \
    TranslationHelper
from openstack_dashboard.test import helpers as test
from openstack_dashboard.usage import quotas

JSON_URL = reverse('horizon:virtual:network_topology:json')
INDEX_URL = reverse('horizon:virtual:network_topology:index')
    ...
    def _test_json_view(self, router_enable=True):
        ...
        expect_server_urls = [
            {'id': server.id,
             ...
             'task': None,
             'url': '/virtual/instances/%s/' % server.id}
        ...
        if router_enable:
            expect_router_urls = [
                {'id': router.id,
                 ...
                 'url': '/virtual/routers/%s/' % router.id}
        ...
        if router_enable:
            expect_net_urls += [{'id': net.id,
                                 'url': '/virtual/networks/%s/detail' % net.id,
                                 'name': net.name,
        ...
        expect_net_urls += [{'id': net.id,
                             ...
                             'subnets': [{'cidr': subnet.cidr,
                                          'id': subnet.id,
                                          'url':
                                          '/virtual/networks/subnets/%s/detail'
                                          % subnet.id}
        ...
        expect_port_urls = [
            {'id': port.id,
             ...
             'url': '/virtual/networks/ports/%s/detail' % port.id}
...
class NetworkTopologyCreateTests(test.TestCase):
    ...
    def _test_new_button_disabled_when_quota_exceeded(
        ...
        self.assertTemplateUsed(res, 'virtual/network_topology/index.html')
    ...
    def test_create_network_button_disabled_when_quota_exceeded(self):
        url = reverse('horizon:virtual:network_topology:createnetwork')
    ...
    def test_create_router_button_disabled_when_quota_exceeded(self):
        url = reverse('horizon:virtual:network_topology:createrouter')
        classes = 'btn btn-default btn-sm ajax-modal'
    ...
    def test_launch_instance_button_disabled_when_quota_exceeded(self):
        url = reverse('horizon:virtual:network_topology:launchinstance')
