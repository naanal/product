...
var push = Array.prototype.push;
  ...
  angular
    ...
    .controller('startRecoveryController', startRecoveryController);

  startRecoveryController.$inject = [
    'recoverInstancesModel', '$scope',
  ...
  function startRecoveryController(recoverInstancesModel,$scope,toast,Spinner,novaAPI,cinderAPI,$interval) {
    ...
    function backupServers(){
      ...
      novaAPI.backupServers({"selectedInstances":recoverInstancesModel.recoverInstancesSpec.selectedInstances})
      .then(function(res){
        if(res.status == 204)
        {
    ...
    function deleteServers(){
      var instances_id = recoverInstancesModel.recoverInstancesSpec.selectedInstances.map(function(obj){return obj.instance_id;});
    ...
    function reBuild(){
      ...
      novaAPI.reCreate({"selectedInstances":recoverInstancesModel.recoverInstancesSpec.selectedInstances}).then(function(res){
        if(res.status == 204)
    ...
    function pullServers(){
      var instances_names = recoverInstancesModel.recoverInstancesSpec.selectedInstances.map(function(obj){return obj.instance_name;});
